...
const AdminClientes = ({ token }) => {
  ...
  const [filtroCursoEnviado, setFiltroCursoEnviado] = useState(false);
  const [filtroPagoConfirmado, setFiltroPagoConfirmado] = useState(false);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filtrosContainer}>
        <div className={styles.filtro}>
          <input
            type="checkbox"
            checked={filtroCursoEnviado}
            onChange={(e) => setFiltroCursoEnviado(e.target.checked)}
          />
          <span>Curso enviado</span>
        </div>
        <div className={styles.filtro}>
          <input
            type="checkbox"
            checked={filtroPagoConfirmado}
            onChange={(e) => setFiltroPagoConfirmado(e.target.checked)}
          />
      ...
      {clientes.length === 0 ? (
        ...
        clientes
          .filter((cliente) => {
            if (filtroCursoEnviado && !cliente.cursoEnviado) return false;
            if (filtroPagoConfirmado && !cliente.pagoConfirmado) return false;
            return true;
          })
          .map((cliente, index) => (
            <div key={index} className={styles.cliente}>
              <div className={styles.info}>
                <p className={styles.dato}>
                  <strong>Nombre:</strong> {cliente.nombreCompleto}
                </p>
                <p className={styles.dato}>
                ...
                <p className={styles.dato} id={styles.enviado}>
                  <strong>Curso Enviado:</strong>{" "}
                  {cliente.cursoEnviado ? (
        ...
        <div className={styles.modalContainer}>
          <div className={styles.modal}>
            ...
            <div className={styles.modalClienteContainer}>
              ...
              <p className={styles.dato}>
                <strong>Precio:</strong>${clienteSeleccionado.precio}
              </p>
            </div>

            {clienteSeleccionado.cursoEnviado ? (
              <p>
                Modificar Curso Enviado a{" "}
